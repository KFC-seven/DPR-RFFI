...
def materialize_records(
    ...
    for record in records:
        ...
        for key in metadata:
            metadata[key].extend([record[key]] * int(signals.shape[0]))
    if not x_parts:
...
def _sample_signals(
    *,
    ...
    if max_samples < 0:
        raise ValueError(f"max_samples_per_record must be nonnegative, got {max_samples}.")
    ...
    record_seed = int.from_bytes(digest[:8], "little") % (2**32)
